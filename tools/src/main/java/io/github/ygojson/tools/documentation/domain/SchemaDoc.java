...
class SchemaDoc {
    ...
    public static final SchemaDoc forModel(final ModelDocumentation modelDoc,
                                           final String schemaPrefix) {
        return new SchemaDoc(modelDoc,
    ...
    private SchemaGenerator createSchemaGenerator() {
        final SchemaGeneratorConfigBuilder builder = new SchemaGeneratorConfigBuilder(SchemaVersion.DRAFT_2019_09, OptionPreset.PLAIN_JSON);
        ...
        // tweak the map defs
        builder.forFields().withCustomDefinitionProvider(SchemaDoc::skipMapDefintions);
    ...
    private String idResolver(final TypeScope scope) {
        return scope.getType().getErasedType() == modelDoc.getModelClass() ? getFullSchemaId() : null;
    }

    private static CustomPropertyDefinition skipMapDefintions(final FieldScope scope,
